...
%macro desynpuf_ip_readin(filenumber=,sortds=);
  ...
  filename inip zip "&infilepath\&infilename.&filenumber..zip" ;

  data desynpuf.&outdsname&filenumber(label="DE1_0_2008_to_2010_Inpatient_Claims_Sample_&filenumber");
    infile inip(&infilename.&filenumber..csv) dsd dlm=',' lrecl=1300 firstobs=2 missover;

    attrib DESYNPUF_ID   length=$16 format=$16. label='Beneficiary Code'
           CLM_ID        length=$15 format=$15. label='IP Claim ID'
           SEGMENT       length=3   format=2. label='IP Claim Line Segment'
           ...
           CLM_ADMSN_DT  length=4   informat=yymmdd8. format=yymmddn8. label='Inpatient admission date'
           ADMTNG_ICD9_DGNS_CD      length=$5 format=$5. label='Claim Admitting Diagnosis Code'
           CLM_PASS_THRU_PER_DIEM_AMT length=8 format=12.2 label='Claim Pass Thru Per Diem Amount'
           NCH_BENE_IP_DDCTBL_AMT     length=8 format=12.2 label='NCH Beneficiary Inpatient Deductible Amount'
           NCH_BENE_PTA_COINSRNC_LBLTY_AM length=8 format=12.2 label='NCH Beneficiary Part A Coinsurance Liability Amount'
           NCH_BENE_BLOOD_DDCTBL_LBLTY_AM length=8 format=12.2 label='NCH Beneficiary Blood Deductible Liability Amount'
           CLM_UTLZTN_DAY_CNT length=3 format=3. label='Claim Utilization Day Count'
           NCH_BENE_DSCHRG_DT length=4 informat=yymmdd8. format=yymmddn8. label='Inpatient discharged date'
           CLM_DRG_CD    length=$3  format=$3. label='Claim Diagnosis Related Group Code'
           ...
           %addseqattrib(varname=HCPCS_CD_,nvars=45,varlength=$5,varformat=$5.,labeltext=Revenue Center HCFA Common Procedure Coding System)
...
/*********************************************************************************/
%desynpuf_ip_readin(filenumber=1,sortds=yes)
%desynpuf_ip_readin(filenumber=2,sortds=yes)
%desynpuf_ip_readin(filenumber=3,sortds=yes)
%desynpuf_ip_readin(filenumber=4,sortds=yes)
%desynpuf_ip_readin(filenumber=5,sortds=yes)
%desynpuf_ip_readin(filenumber=6,sortds=yes)
%desynpuf_ip_readin(filenumber=7,sortds=yes)
%desynpuf_ip_readin(filenumber=8,sortds=yes)
%desynpuf_ip_readin(filenumber=9,sortds=yes)
%desynpuf_ip_readin(filenumber=10,sortds=yes)
%desynpuf_ip_readin(filenumber=11,sortds=yes)
%desynpuf_ip_readin(filenumber=12,sortds=yes)
%desynpuf_ip_readin(filenumber=13,sortds=yes)
%desynpuf_ip_readin(filenumber=14,sortds=yes)
%desynpuf_ip_readin(filenumber=15,sortds=yes)
%desynpuf_ip_readin(filenumber=16,sortds=yes)
%desynpuf_ip_readin(filenumber=17,sortds=yes)
%desynpuf_ip_readin(filenumber=18,sortds=yes)
%desynpuf_ip_readin(filenumber=19,sortds=yes)
%desynpuf_ip_readin(filenumber=20,sortds=yes)
